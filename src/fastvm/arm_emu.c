...
int arm_insteng_add_exp(struct arm_inst_engine *en, const char *exp, arm_inst_func func)
{
    ...
    int j, len = strlen(exp), rep, height;
    const char *s = exp;
    ...
    for (; *s; s++) {
        ...
        case '0':
            if (!root->left) {
                root->left = inst_node_new(&en->enfa, root);
            }
        ...
        case '1':
            if (!root->right) {
                root->right = inst_node_new(&en->enfa, root);
            }
        ...
        case 'l':
            ...
            for (j = 0; j < rep; j++) {
                root->rep = inst_node_new(&en->enfa, root);
                root = root->rep;
            }
    ...
    while (!istack_is_empty()) {
        ...
        for (i = 0; i < 3; i++) {
            ...
            if (nroot->left || nroot->rep) {
                dnode1 = droot->left = inst_node_new(&eng->dfa, droot);

                droot->left->set[0] = nroot->left;
                droot->left->set[1] = nroot->rep;
            }
            else if (nroot->right || nroot->rep) {
                dnode1 = droot->right = inst_node_new(&eng->dfa, droot);
