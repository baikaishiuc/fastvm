...

{
  if ((op->code() == CPUI_CALL) || (op->code() == CPUI_CALLIND))
  {
    ...
    if (fc == (FuncCallSpecs *)0)
      return true; // Assume indirect effect
...

{
  if (addr.getSpace() != spc)
    return false;
  if (addr.getOffset() < minimumOffset)
    return false;
  if (addr.getOffset() > maximumOffset)
    return false;
  return true;
}
  ...
  for (int4 i = 0; i < freeStores.size(); ++i)
  {
    ...
    while (indOp != (PcodeOp *)0)
    {
      ...
      if (iopVn->getSpace()->getType() != IPTR_IOP)
        break;
      if (op != PcodeOp::getOpFromConst(iopVn->getAddr()))
        break;
...

{
  ...
  if (read.empty() && write.empty() && inputvars.empty())
    return;
...

{
  ...
  for (int4 i = 0; i < fd->numCalls(); ++i)
  {
    ...
    if (fc->isInputActive())
    {
      ...
      {
        if (fc->getStackPlaceholderSlot() < 0)
        { // Any stack resolution is complete (or never started)
        ...
        { // Stack has not been resolved, so we need to abort
...

{
  ...
  while (iter != loadGuard.end())
  {
    ...
    ++iter;
    if (guardRec.spc != addr.getSpace())
      continue;
    if (addr.getOffset() < guardRec.minimumOffset)
      continue;
    if (addr.getOffset() > guardRec.maximumOffset)
      continue;
    ...
    if (fd->getFuncProto().possibleOutputParam(addr, size))
    {
      ...
      for (iter = fd->beginOp(CPUI_RETURN); iter != iterend; ++iter)
      {
        ...
        if (op->isDead())
          continue;
        if (op->getHaltType() != 0)
          continue; // Special halt points cannot take return values
        Varnode *invn = fd->newVarnode(size, addr);
        invn->setActiveHeritage();
...

{
  ...
  int4 lastpos = 0;
  for (int4 curpos = 1; curpos < size; ++curpos)
...

{
  ...
  if ((input.size() == 1) && (input[0]->getSize() == size))
    return;
...

{
  ...
  {
    Varnode *vn = *iter++;
    if (vn->getSpace() != joinspace)
      break; // New varnodes may get inserted before enditer
    ...
    while (iter != enditer)
    {
      ...
      { // If completely contained in range from previous pass
        if (vn->isHeritageKnown())
          continue; // Don't heritage if we don't have to
        if (vn->hasNoDescend())
          continue;
      ...
      else
      {
        if ((!needwarning) && (info->deadremoved > 0))
        {
          ...
          if (vn->isHeritageKnown())
            continue; // Assume that it is tiled and produced by merging
