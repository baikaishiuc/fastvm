                                                ...
                                                OpTpl *op, VarnodeTpl *vn, bool isbigendian)
{
  ...
  if (off.getType() != ConstTpl::handle)
    return true;
  if (off.getSelect() != ConstTpl::v_offset_plus)
    return true;
...
{ // Conservatively test whether vn1 and vn2 can intersect
  ...
  if (vn1->getSpace().getType() != ConstTpl::spaceid)
    return true;
  if (vn2->getSpace().getType() != ConstTpl::spaceid)
    return true;
  AddrSpace *spc = vn1->getSpace().getSpace();
  if (spc != vn2->getSpace().getSpace())
    return false;

  if (vn2->getOffset().getType() != ConstTpl::real)
    return true;
  if (vn2->getSize().getType() != ConstTpl::real)
    return true;

  if (vn1->getOffset().getType() != ConstTpl::real)
    return true;
  if (vn1->getSize().getType() != ConstTpl::real)
    return true;
...

{
  ...
  for (int4 i = 0; i < ct->getNumOperands(); ++i)
  {
    ...
    if (collideOperand >= 0)
    {
      ...
      {
        reportWarning(getLocation(ct), "Possible operand collision between symbols '" + ct->getOperand(collideOperand)->getName() + "' and '" + ct->getOperand(i)->getName() + "'");
      }
...

{
  ...
  if (collisionCount > 0)
  {
    ...
    if (!warnalllocalcollisions)
      msg << endl
          << "Use -c switch to list each individually";
    reportInfo(msg.str());
  }
}
...
{ // Make sure label symbols are used properly
  ...
  for (iter = scope->begin(); iter != scope->end(); ++iter)
  {
    ...
    if (sym->getType() != SleighSymbol::label_symbol)
      continue;
...
{ // Match up any qualities of the macro's OperandSymbols with
  ...
  for (uint4 i = 0; i < param.size(); ++i)
  {
    ...
    if (outvn->getOffset().getType() != ConstTpl::handle)
      continue;
...
{ // Shift the offset up by -sa- bits, for either the dynamic or static varnode aspects that are in the unique space
  if (hand->getSpace().isUniqueSpace() && (hand->getPtrSpace().getType() == ConstTpl::real) && (hand->getPtrOffset().getType() == ConstTpl::real))
  {
